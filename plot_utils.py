...
def plot_dots(ax,X,Y, title='Source data', alpha=0.8, s=20, marker='o'):
    ...
    for i in range(nc):
        ...
        inds_ = np.where(Y==L[i])[0]
        ax.scatter(X[inds_, 0], X[inds_, 1],edgecolor='black',c=colors[i],label='class '+str(i),alpha=alpha, s=s, marker=marker)
...
def plot_dec_func(ax, clf, x_min, x_max, y_min, y_max, plot_step = 0.01,alpha=0.8,cmap="tab10",title='Source data',nc=4):
    ...
    ax.contour(xx, yy, ypred_src, levels=nc, linewidths=0.5, colors='k')
    ax.contourf(xx, yy, ypred_src, levels=nc, colors=colors, alpha=alpha)
